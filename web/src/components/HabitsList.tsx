...
interface HabitsListProps {
  date: Date;
  onToggleCompleted: (completed: number) => void;
}
...
export function HabitsList({ date, onToggleCompleted }: HabitsListProps) {
  ...
  async function handleToggleHabit(habitId: string) {
    ...
    onToggleCompleted(completedHabits.length);
      ...
      {habitsInfo?.possibleHabits?.map((habit) => {
        return (
          <Checkbox.Root
            ...
            className="flex items-center gap-3 group focus: outline-none disabled:cursor-not-allowed disabled:opacity-60"
          >
