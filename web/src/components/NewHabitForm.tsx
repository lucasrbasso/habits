...
export const NewHabitForm = () => {
  ...
  function handleToggleWeekDay(weekDay: number) {
    if (weekDays.includes(weekDay)) {
      setWeekdays((previousState) =>
        previousState.filter((day) => day !== weekDay)
      );
    } else {
      setWeekdays((previousState) => [...previousState, weekDay]);
    }
  }
  ...
  return (
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
      ...
      <button
        type="submit"
        className="mt-6 rounded-lg p-4 flex items-center gap-3 font-semibold bg-green-600 hover:bg-green-500 transition-colors ease-in-out delay-100 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 focus:ring-offset-zinc-900 "
      >
